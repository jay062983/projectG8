...
public class APIWorkflowSteps {
    ...
    public void a_request_is_prepared_to_create_an_employee_using_json_payload() {
        request = given().header(APIConstants.HEADER_KEY_CONTENT_TYPE, APIConstants.HEADER_VALUE_CONTENT_TYPE).
                header(APIConstants.HEADER_KEY_AUTHORIZATION, GenerateTokenSteps.token).
                body(APIPayloadConstants.createEmmployeePayloadJson());
    }
    ...
    @Then("the status code for creating an emplouee is {int}")
    public void the_status_code_for_creating_an_emplouee_is(Integer int1) {
    ...
    public void the_employee_id_is_stored_as_a_global_variable_to_be_used_for_other_calls(String string) {
        //stroing the employee id in global variable
    ...
    public void the_retrieved_data_at_object_matches_with_the_data_of_created_employee(String empObject, DataTable dataTable) {
        ...
        public void a_request_is_prepared_to_create_an_employee_with_dynamic_data
            ...
            request= given().
                    ...
                    header(APIConstants.HEADER_KEY_AUTHORIZATION,GenerateTokenSteps.token).
                    body(APIPayloadConstants.createEmmployeePayloadJsonDynamic(firstName,lastName,middleName,gender,birthday,status,jobTitle));
        }
    @Given("a request is prepated to update an employee")
    public void a_request_is_prepated_to_update_an_employee() {
       request= given().
               header(APIConstants.HEADER_KEY_CONTENT_TYPE,APIConstants.HEADER_VALUE_CONTENT_TYPE).
               header(APIConstants.HEADER_KEY_AUTHORIZATION,GenerateTokenSteps.token).
               body(APIPayloadConstants.updateEmmployeePayloadJson());
    }
